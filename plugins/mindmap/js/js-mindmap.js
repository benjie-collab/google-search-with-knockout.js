...
(function($){
    ...
    $.fn.mindmap = function(options) {
        ...
        function Node(obj, index, el, parent, active){
          ...
          this.el.css('top', this.y + "px");
            ...
            var thisnode = this;
            this.el.dblclick(function(){ //click || dblclick
 //               console.log(obj.activeNode);
            ...
            this.el.children('.node-info').children('.node-info-action.node-show-children').click(function(){ //click || dblclick
                if (obj.activeNode) obj.activeNode.el.removeClass('active');
                obj.activeNode = thisnode;
                obj.activeNode.el.addClass('active');
                return false;
			...
			this.el.children('.node-info').hover(				
				function (){
					clearTimeout(timer);
				},
				function(){				
			...
			var maxpphcontent = 250;
			this.el.children('.node-info').children('.node-info-action[rel=popover]').click(function(){ //click || dblclick              
				var pp = this;
				$(pp).closest('.node-info').find('.popover').remove();
				$(pp).popover({
					html:true,
					placement:"top",
					trigger: 'manual',
					placement: 'right',
					container: $(pp).closest('.node-info').get(0),
					offset:5,
					delay:{ show:500, hide:100 },
					template:'<div class="popover"><div class="arrow"></div><h2 class="popover-title"></h2><div class="popover-content" style="max-height: ' + maxpphcontent + 'px"><div></div></div></div>',
					title:function () {
						return $(pp).attr('data-title') + '<button class="pull-right btn btn-xs btn-transparent" type="button" onclick="$(this).closest(' + "'" + '.popover' + "'" + ').remove()">&times;</button>';
					},
					content:function () {
						var which = '<div class="scrollable" data-height="250">' +$(pp).find('.node-info-action-popover-content').html() + '</div>';              
						return which;
					}
				})
				.popover('show');
				...
				$(".scrollable").each(function(){	
					if(parseInt(parseInt($(this).attr('data-height'))-10 || maxpphcontent-10) < $(this).height())
					$(this).slimscroll({
						height: parseInt($(this).attr('data-height'))-10 || maxpphcontent-10,
						size: '5px',
						alwaysVisible: true,
						railVisible: true
					});	
...
//                console.log(this);
        ...
        return this.each(function() {
            ...
            if (typeof window.CanvasRenderingContext2D == 'undefined' && typeof G_vmlCanvasManager == 'undefined') {
                ...
                //create element
                this.canvas = $('<canvas width="'+(options.mapArea.x - 40)+'" height="'+options.mapArea.y+'" style="position:absolute;left:0;top:0;"></canvas>');
                //add to document
                $(this).append(this.canvas);
                
                //NODES
                // create root node
                var myroot = $("div.mind-map-root", this).get(0);
                var nodeno = nodes.length;
