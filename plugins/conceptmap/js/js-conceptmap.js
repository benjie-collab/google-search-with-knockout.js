function conceptmapModel(opts){	
	...
	var defaultInfo = '<div id="default-info" class=""><h2 class="text-center"><img src="images/icons/puzzle.png"/></h2>' +
			'<h5 class="text-center">Can\'t find what you are searching for? Try this conceptual map of Documents against Commenters and Topics</h5>' +
			'<p>Quis autem vel eum iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur, vel illum qui dolorem eum fugiat quo voluptas nulla pariatur?</p>' +
			'<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt.</p>' +		
			'</div>';
	
	
    var a = $(opts.container).width() - 10,
        c = 800,
        h = c,
        U = 200,
        K = 22,
    ...
    var d = d3.select(opts.container).append("svg").attr("width", a).attr("height", c).append("g").attr("transform", "translate(" + a / 2 + "," + c / 2 + ")");
    ...
    var Q = d3.select("#results-container");
	
    d3.json(self.settings().sitefolder + '/plugins/conceptmap/data/data.json', function (X, Y) {
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        A = d3.map();
    ...
    function M() {
        ...
        n();
        z();
        //$("#conceptmap_search_result").find(".search-count").html("0");
    }
    ...
    function e() {
        ...
        if (L.node.description) {
        	/*
        	Y= "<div class=\"media\">";
        	Y+= "<a title=\"Comment\" data-toggle=\"tooltip\" class=\"pull-left right-tooltip\" ><span class=\"document-list-source pull-left facebook\"></span></a> <div class=\"media-body\">";
        	Y+= "<h6 class=\"media-heading document-title\">" + L.node.name + "</h6>";*/
            
            
           // console.log(L.node);
            /*
            if(L.node.comments && L.node.type == "theme"){            	
            	L.node.comments.forEach(function (_com) {          		
            		Y+= "<div class=\"media\">";
            		Y+= "<a title=\"Comment\" data-toggle=\"tooltip\" class=\"pull-left right-tooltip\" ><span class=\" pull-left icon-user\"></span></a> <div class=\"media-body\">" + _com + "</div>";
            		Y+="</div>";
            	});             	    	
            }
            Y+="</div></div>";*/
             ...
             * Type: theme = Document
             * Type: epsiode = Author/User
             * Type: perspective = Topic
             * 
             */
        	//Y= L.node.description;
        	//$('.page.sliding-page').addClass('openpage');
        	
        	
        	
        	
        	
            if(L.node.type == "theme"){ 
            	/*
            	 * Document
               _concept_Map_Get_Document(
            		   {
	            			nodetype: "theme",
	            			query: "*",
	            			uri: L.node.name
            	...
            	json_searchOpts['uri'] = L.node.name;
            	//$.fn.basicDocumentSearch();
               
            }
            
            else if(L.node.type == "episode"){  
            	
            	/*
            	 * Profiles
	            _concept_Map_Get_Documents(
	            		{
	            			nodetype: "episode",
	            			query: "*",
	            			commenter: L.node.name,
	            			page: page,
	            			limit: pagelimit
	            		}
	            );*/
            	...
            	//json_searchOpts.uri = 
            	json_searchOpts['profile'] = L.node.name;
            	//$.fn.basicDocumentSearch();
            }
            
            else if(L.node.type == "perspective"){ 
            	/*
            	 * Topic
	            _concept_Map_Get_Documents(
	            		{
            	...
            	json_searchOpts[(L.node.entity).toLowerCase()] = L.node.entityfull;
            	//$.fn.basicDocumentSearch();
            }
        }
        Q.html(Y);
    ...
    function b(X) {
        ...
        X.selectAll("text").transition().duration(w).ease(F).attr("dy", ".3em").attr("font-size", function (Z) {
        	...
        	if(Z.type== 'theme')
        		return (Z.name).substring(Z.name.length-10, Z.name.length);
        	else
        		return Z.name
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "episode") {
            ...
            if (ab && ab.type === "theme") {
                ...
                if (ab && ab.type === "perspective") {
                    ...
                    Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("ENTITY");
                    ad.exit().remove()
                }
            }
        }
        ...
        X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).text("Go to Root").attr("class", "all-episodes").on("click", O);
        X.exit().remove()
    }
            ...
            if (X === L.node) {
                ...
                if (X.type === "theme") {
                    ...
                    if (X.type === "perspective") {
                    	...
                    	return self.entityColor[(X.entity).toLowerCase()];
